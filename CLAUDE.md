...
| JaCoCo fails with `IllegalArgumentException: Unsupported class file major version 69` during `jacocoTestReport` | JaCoCo ≤ 0.8.12 predates Java 25 and rejects bytecode at class-file major version 69 (produced by javac 25). | Pin `jacoco { toolVersion = '0.8.14' }` (or newer) in the root `build.gradle`'s `subprojects` block. 0.8.14 (2025-09) is the first release with Java 25 support. |
| PIT `excludedClasses` Ant-glob like `'**/*package-info*'` matches nothing | PIT class filters use **dotted** class-name globs (`*` matches any char in an FQN), NOT Ant filesystem paths. `**/` is a literal that never appears in a dotted name. | Use dotted form: `'com.skyflux.kiln.<mod>.*.package-info'` or `'com.skyflux.kiln.<mod>.**.package-info'`. Silently no-op today (PIT skips package-info bytecode) but bites the moment someone writes `'**/*Service*'` expecting an exclusion. |
| Code referencing `org.springframework.boot.web.context.WebServerApplicationContext` fails to compile with `package ... does not exist`, even though Boot 4 is on the classpath | The Boot 3 `WebServerApplicationContext` in `org.springframework.boot.web.context` was part of the monolithic `spring-boot` artifact, which no longer exists in Boot 4. Boot 4 introduces a **new** `WebServerApplicationContext` interface at `org.springframework.boot.web.server.context` (`@Since("4.0.0")`), shipped by the separate `spring-boot-web-server` module. There is no deprecated bridge — the old import simply stops resolving. | Rewrite the import as `org.springframework.boot.web.server.context.WebServerApplicationContext`. `ConfigurableWebServerApplicationContext` moved the same way. `WebServer` itself stayed at `org.springframework.boot.web.server.WebServer` (same package in Boot 3 and 4). |
| Context startup fails: `@TransactionalEventListener method must not be annotated with @Transactional unless when declared as REQUIRES_NEW or NOT_SUPPORTED`. | Spring's `RestrictedTransactionalEventListenerFactory` refuses plain `@Transactional` on an `@TransactionalEventListener` method — at AFTER_COMMIT the parent tx is in the process of closing, so the listener body cannot enrol in it; a new independent tx is mandatory. Intuition "REQUIRES_NEW is redundant because we're already outside the parent" is wrong. | On every `@TransactionalEventListener` whose body touches DB, declare `@Transactional(propagation = Propagation.REQUIRES_NEW)` (or `NOT_SUPPORTED` if the body is read-only). |
| `./gradlew build` flaky on Colima: `ContainerLaunchException: Container startup failed for image testcontainers/ryuk:0.14.0` during test-context load, OR `IllegalStateException: LettuceConnectionFactory has been STOPPED` mid-test. Passes in isolation (`:app:test --tests FooTest`), fails in aggregate builds. | User-local `~/.testcontainers.properties` `ryuk.disabled=true` is read by the Testcontainers SDK when **directly invoked from shell**, but Gradle's forked `test` JVMs and custom `JavaExec` tasks don't reliably inherit that state across CI machines or teammates. When Ryuk tries to bind-mount `~/.colima/default/docker.sock`, Colima rejects the mount (`operation not supported`), tearing down the whole context — including the still-booting `LettuceConnectionFactory`. | Set `environment 'TESTCONTAINERS_RYUK_DISABLED', 'true'` on BOTH the shared `test { }` block (root `build.gradle`, `subprojects`) AND on every custom `JavaExec` task that boots a Testcontainers context (e.g. `updateOpenApiSnapshot` in `app/build.gradle`). The env-var form propagates through Gradle's fork semantics more reliably than `systemProperty 'testcontainers.ryuk.disabled'`. |

| jOOQ DDLDatabase produces 0 tables when `scripts` is a comma- or semicolon-separated list of absolute file paths | jOOQ's `FilePattern` does not treat comma-separated absolute paths the same as a directory scan. Each entry is resolved as a glob against a working directory, so absolute paths silently match nothing, and DDLDatabase generates 0 tables with no error. Observed when switching from directory form to explicit list to exclude `R__*.sql` from codegen. | Use a **glob pattern** instead: `"${project.projectDir}/src/main/resources/db/migration/V*.sql"`. This matches only versioned migrations (`V1__…` through `V7__…`) and automatically excludes `R__*.sql`. The trailing-slash directory form still works but includes ALL files including `R__*.sql`. |
| `ALTER TABLE … ADD COLUMN IF NOT EXISTS` causes jOOQ DDLDatabase to FAIL (SEVERE) and abort schema export | H2's DDL parser (used internally by jOOQ DDLDatabase) does not support `IF NOT EXISTS` on `ADD COLUMN`. jOOQ translates PG `ALTER TABLE ADD COLUMN` but not the `IF NOT EXISTS` variant. The failure is a hard SEVERE/exception, not silent. | Remove `IF NOT EXISTS` from `ADD COLUMN` statements in V-migrations. Since Flyway versioned migrations run exactly once, idempotency is not needed here. Move any PG-specific DML (`ON CONFLICT`, `ENABLE ROW LEVEL SECURITY`, `CREATE POLICY`, backfill `UPDATE`s, NOT NULL enforcement after backfill) to `R__*.sql`. |

| `./gradlew :infra:generateJooq` emits `WARNING Ambiguous key name` when two FK paths produce the same implicit join method name (e.g. direct `users.tenant_id→tenants` + chained `users→user_roles→roles→tenants` both generate `tenants()` on `UsersPath`) | jOOQ DDLDatabase generates implicit join path shortcuts for every FK chain; when two chains converge on the same target table, the generated method name collides and jOOQ warns but still emits broken code. | In `infra/build.gradle`'s `generate {}` block add `implicitJoinPathsToMany = false`. This disables the to-many chained-FK shortcuts (the source of the ambiguity) while leaving Record types, direct to-one FK accessors, and relations intact. Note: `implicitJoinPathsManyToMany` and `implicitJoinPaths` are NOT the correct property names in jOOQ 3.20 Groovy DSL — only `implicitJoinPathsToMany` works. |
